...
const RecentTx = () => {
  ...
  useEffect(() => {
    async function getData() {
      const tx = await App.EtherPayContract.filters.transactions(App.Address)
      const txData = await App.EtherPayContract.queryFilter(tx);
      setData(txData)
    }
      ...
      {data.map((e) => {
        return (
          <div className={`bg-black rounded-lg bg-opacity-60 border-2 border-blue-900 border-opacity-80 w-4/5 mt-2`}>
            ...
            <a target={'_blank'} href={`https://goerli.etherscan.io/tx/${e.transactionHash}`}>
